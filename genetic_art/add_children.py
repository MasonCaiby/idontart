"""DOCSTRING
This was my first implementation of genetic programming. It just takes parents and mutates them. The successful
mutations for each generation create the queue for parents in the next generation as well as the minimum between
[100, 30% of the number of shapes we have in total].
...
class NewImage():
    ...
    def mutate_shape(self, shape):
        ...
        for _ in range(self.num_children):
            ...
            new_color = tuple(int(color + color * self.rand_var()) for color in shape[1][:3])
            new_color += (125,)
    ...
    def compare_new_images(self, new_image):
        new_diff = sum(np.fabs(np.subtract(self.true_image.getdata(), new_image.getdata())).flatten())
...
if __name__ == "__main__":
    ...
    new_image.init_shapes(num_shapes=3000, sides=3)
